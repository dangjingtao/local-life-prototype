...
function Bottle({ slim = false }: { slim?: boolean }) {
  return (
    <div className={`relative shrink-0 ${slim ? "h-[70%] w-[24%]" : "h-[76%] w-[31%]"}`}>
      ...
      >
        <div className="absolute inset-x-[14%] top-[35%] rounded-sm bg-white/90 px-1 py-1 text-center">
          <div className="mx-auto h-1 w-2/3 rounded-full bg-[var(--mall-art-accent)]/40" />
          <div className="mx-auto mt-1 h-1 w-1/2 rounded-full bg-[var(--color-text-primary)]/20" />
        </div>
      </div>
    </div>
  );
...
function Box({ tall = false, label = "SELECT" }: { tall?: boolean; label?: string }) {
  return (
    <div
      className={`relative shrink-0 overflow-hidden rounded-[10%] border border-white/70 bg-white/90 shadow-sm ${tall ? "h-[82%] w-[42%]" : "h-[66%] w-[48%]"}`}
    >
      ...
      <div className="absolute left-[12%] right-[12%] top-[34%]">
        ...
        <div className="mt-1 h-1 w-3/5 rounded-full bg-[var(--color-text-primary)]/10" />
      </div>
      <div className="absolute -bottom-[20%] -right-[12%] h-[54%] w-[54%] rounded-full bg-[var(--mall-art-accent)]/20" />
    </div>
  );
}
...
function Cup() {
  return (
    <div className="relative h-[66%] w-[34%] shrink-0">
      <div className="absolute left-1/2 top-0 h-[8%] w-[72%] -translate-x-1/2 rounded-full bg-[var(--color-text-primary)]/50" />
...
export function MallProductArtwork({ productId, name, className = "", decorative = false }: MallProductArtworkProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute -bottom-[24%] -left-[10%] h-[54%] w-[54%] rounded-full bg-[var(--mall-art-accent)]/10" />
      ...
      <div className="absolute inset-x-[12%] bottom-[4%] h-[5%] rounded-full bg-[var(--color-text-primary)]/10 blur-[4px]" />
    </div>
  );
}
