...
import { MerchantSmartCareOperations, type CareAppointmentOverrides, type CareScanOverrides, type CareSlotAvailabilityOverrides } from "./SmartCareOperations";
import {
  CORE_DEMO_IDS,
  businessSceneLabels,
  coupons,
...
type Page = "workspace" | "fulfillment" | "care" | "orders" | "members";
...
const pageCatalog: Record<Page, { label: string; icon: "home" | "modules" | "profile" }> = {
  ...
  fulfillment: { label: "便利店履约", icon: "modules" },
  care: { label: "智慧抗衰运营", icon: "modules" },
...
const merchantConvenienceOrders = v02Orders.filter((order) =>
  order.scene === "store" &&
  order.storeId === CORE_DEMO_IDS.store &&
  (order.fulfillmentDetail?.mode === "pickup" || order.fulfillmentDetail?.mode === "short_delivery"),
);
...
function MerchantOverview({ onNavigate, onPermission, overrides, fulfillmentOverrides }: { onNavigate: (page: Page) => void; onPermission: () => void; overrides: RedemptionOverrides; fulfillmentOverrides: FulfillmentOverrides }) {
  ...
  const conveniencePending = fulfillmentStatuses.filter((status) =>
    status === "preparing" || status === "ready_for_pickup" || status === "delivering"
  ).length;

  return <><section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]"><div className="rounded-[var(--radius-overlay)] bg-[var(--color-primary)] p-6 text-white md:p-7"><div className="flex flex-wrap items-center justify-between gap-3"><StatusTag>店主 / 合作商</StatusTag><span className="text-xs text-white/65">T022 · FR-501 / FR-502</span></div><h2 className="mt-6 text-2xl font-semibold md:text-3xl">{coreDemoStore.name}经营工作台</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/75">处理本店便利店履约、智慧抗衰预约核销与报告承接，并查看与本店业务直接关联的用户。其他门店明细不会进入当前视图。</p><div className="mt-6 grid gap-3 border-t border-white/15 pt-4 text-sm sm:grid-cols-2"><div><span className="text-white/55">合作商</span><strong className="mt-1 block font-medium">{merchantPartner?.name ?? coreDemoStore.partnerId}</strong></div><div><span className="text-white/55">门店范围</span><strong className="mt-1 block font-medium">{coreDemoStore.id} · {coreDemoStore.address}</strong></div></div></div><Card className="p-5"><div className="flex items-center justify-between"><h3 className="font-semibold">当前授权</h3><StatusTag tone="success">仅本店</StatusTag></div><p className="mt-4 text-sm leading-6 text-[var(--color-text-secondary)]">{pcDataScopeLabels.assigned_store}：{coreDemoStore.name}</p><div className="mt-5 space-y-2 border-t border-[var(--color-border)] pt-4 text-sm text-[var(--color-text-secondary)]"><p>✓ 可查看本店便利店履约、智慧抗衰预约 / 报告和关联用户</p><p>— 不展示南岸生活馆等其他门店明细</p></div><SecondaryButton className="mt-5 w-full" onClick={onPermission}>演示越权状态</SecondaryButton></Card></section><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{metrics.map((metric) => <Card key={metric.label}><p className="text-sm text-[var(--color-text-secondary)]">{metric.label}</p><p className="mt-3 text-2xl font-semibold">{metric.value}</p><p className="mt-2 text-xs text-[var(--color-text-tertiary)]">{metric.note}</p></Card>)}</div><Section title="快捷处理"><div className="grid gap-4 lg:grid-cols-3"><button type="button" onClick={() => onNavigate("fulfillment")} className="rounded-[var(--radius-container)] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 text-left transition hover:border-[var(--color-primary)]"><div className="flex items-center justify-between"><div className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-control)] bg-[var(--color-brand-subtle)] text-[var(--color-primary-pressed)]"><PrototypeIcon name="modules" size={19} /></div><StatusTag tone="warning">{conveniencePending} 待处理</StatusTag></div><h3 className="mt-4 font-semibold">便利店履约</h3><p className="mt-2 text-sm leading-6 text-[var(--color-text-secondary)]">查看本店自提 / 短配订单，演示扫码核销和配送状态推进。</p><p className="mt-4 text-sm font-medium text-[var(--color-primary-pressed)]">进入履约 →</p></button><button type="button" onClick={() => onNavigate("orders")} className="rounded-[var(--radius-container)] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 text-left transition hover:border-[var(--color-primary)]"><div className="flex items-center justify-between"><div className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-control)] bg-[var(--color-brand-subtle)] text-[var(--color-primary-pressed)]"><PrototypeIcon name="modules" size={19} /></div><StatusTag tone="warning">{pendingRedemptions} 待核销</StatusTag></div><h3 className="mt-4 font-semibold">服务与历史核销</h3><p className="mt-2 text-sm leading-6 text-[var(--color-text-secondary)]">保留既有自提 / 体验凭证演示，便于 V0.1 回归。</p><p className="mt-4 text-sm font-medium text-[var(--color-primary-pressed)]">进入处理 →</p></button><button type="button" onClick={() => onNavigate("members")} className="rounded-[var(--radius-container)] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 text-left transition hover:border-[var(--color-primary)]"><div className="flex items-center justify-between"><div className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-control)] bg-[var(--color-brand-subtle)] text-[var(--color-primary-pressed)]"><PrototypeIcon name="profile" size={19} /></div><StatusTag>{merchantUsers.length} 位关联用户</StatusTag></div><h3 className="mt-4 font-semibold">门店用户概览</h3><p className="mt-2 text-sm leading-6 text-[var(--color-text-secondary)]">仅查看与本店订单、体验或核销发生关联的演示用户。</p><p className="mt-4 text-sm font-medium text-[var(--color-primary-pressed)]">查看本店用户 →</p></button></div></Section></>;
}
