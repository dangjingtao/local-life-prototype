...
test.describe("T028 · expanded convenience browse UX", () => {
  ...
  test("bundle and single modes have clearly different product presentations", async ({ page }) => {
    await openYunling(page);

    await expect(page.getByRole("button", { name: "单品", exact: true })).toHaveAttribute("aria-pressed", "true");
    await expect(page.getByRole("region", { name: "单品双栏浏览" })).toBeVisible();

    await page.getByRole("button", { name: "套餐", exact: true }).click();
    await expect(page.getByRole("button", { name: "套餐", exact: true })).toHaveAttribute("aria-pressed", "true");
  ...
  test("activity carousel and coupon entry are consumer-facing and interactive", async ({ page }) => {
    await openYunling(page);

    const carousel = page.getByRole("region", { name: "门店活动轮播" });
    await expect(carousel).toContainText("早八能量补给");
    await page.getByRole("button", { name: "切换到活动 2" }).click();
    ...
    await expect(page.getByText("门店 10 元优惠券", { exact: true })).toBeVisible();
