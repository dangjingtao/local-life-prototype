...
- OpenRouter API only for the experimental PR review workflow, with the credential stored as repository secret `OPENROUTER_API_KEY`
...
- Configure and run the experimental AI review workflow for same-repository PRs targeting `dev`
- Post or update the single advisory AI review comment generated by that workflow
